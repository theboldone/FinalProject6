...
library(openintro)
library('rsconnect')
#rsconnect::setAccountInfo(name='eni4q3-manasi-danke', token='5D371553F1FBF0EB770672B272C59705', secret='K9i+aAhNiKHB9yD/xped06AWvODncw+ewrQgD/W8')
#library(rsconnect)
#rsconnect::deployApp('/Users/manasidanke/Documents/5849248/5836943/shiny')

#values to be saved and updated
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "x", 
                  ...
                  choices = c("Age", "YearsExperience", "Salary"), 
                  selected = "Age"),
      sliderInput("integer", "Training Set (0.8 is 80% for Training and 20% for Testing):",
                  min = 0.3, max = 0.85,
                  value = 0.8),
    ),
...
"
  ...
  output$text1.3 <- renderText({
    "Insights and Results: When we train the model with 80% data in the training set and 20% in the test set, we are able to calculate 2 linear regression models for Age and Salary and also YearsExperience and Salary. Looking at Figure 3 where we have applied the model found by training, we can understand the trend between Age or YearsExperience with Salary. We were able to use the model to predict the y-values for Salary(when y-value is Salary that has been selected) with the test set. The R-squared values for Age predicting Salary and for YearsExperience predicting Salary are very high at (0.940) and (0.949) respectively, close to 1. This means that the model is very accurate and can help us more accurately explain the variability of y.  My hypothesis is correct that there is a strong relationship between Age predicting Salary and for YearsExperience predicting Salary. Age or YearsExperience individually are strong factors to predict Salary. The Root Mean Squared Error is around $6100 which is great, considering there are salary values of six figure. That means the average distance between the predicted salary value and the actual value is $6100, based on the experiment I recently ran. Given this information, I can generally provide my Age or YearsExperience to estimate what I should expect to be paid and see the trend of that overtime. This insight lets me more accurately estimate my salary, advocate for a fair number for myself and other jobseekers, and educates employers and recruiters to pay underpaid workers a fair price. 
"
  })
  output$text2.1 <- renderText({
    "Model Chosen: The model I chose was linear regression and I wanted to compare the input value of Age to see how it affects the output value Salary. I also wanted to see how the input value of YearsExperience (Years of Experience) affects the output value Salary. With the way I have designed the application, you can explore these selected x,y or input, output values and check the relationship of any 2 combinations of these 3 variables. Regression also shows the trend of these 2 combinations of input and output values, such as if they move in opposite directions where the value of one variable increases and the other decreases. For my purposes, I want to predict the y value I am interested in (Salary) based on either Age or YearsExperience and understand the general trend between either of Age or YearsExperience x values and y values of Salary which I am most interested in. By originally plotting the scatterplot, I saw that the relation between Age and Salary was linear and the relation between YearsExperience and Salary was linear. The correlation value was also close to 1 so I knew this linear regression model has a strong positive linear relationship. 
"
  })
  output$text2.2 <- renderText({
    "The Algorithm Explained: Through linear regression with one input variable for x and one output variable for y, we want to see the trend between these two variables (for example: both increase positively or in opposite directions when the value for a variable grows larger). We also want to create a model that can help us predict the relationship between x and y and understand the strength at which x can be used to predict y when using the model and the general variance or deviation of values from predicted vs what they are listed and observed as in reality. By collecting x and y pairs of data showing what x value is there for the y value and generating a robust dataset, I strongly support plotting the scatterplot of values to see if there is a linear relationship (such as where a line can pass through most of the points or a similar number of points are on or above and below the line). I also look at the correlation value and see if it is close to 1 to see an early indicator of a strong linear relationship between the x and y values. In order to train our model for prediction, we need to divide it into training dataset and testing dataset. With 80% of our dataset for training, we can look to establish a model through y=ax+b+e where y is the dependent variable, a is the slope, b is the intercept, x is the independent variable, and e is the error or residual (which is the the difference of the predicted from listed/observed value in reality or (observed value -predicted value)). 

"
  })
  output$text2.3 <- renderText({
    "The lm() function in r helps estimate a and b and generates the linear model by taking in the x and y variables and training data set. linear_regression = lm(y ~ x, data=trainingData) That helps to get the predicted y value and thus use the least squares method. The least squares method version of linear regression works to find the line of best fit and minimizes the predicted residual sum of squares, called PRESS. We want a small PRESS value since that would mean the model predictive ability is better than if the PRESS value was larger. PRESS = sum((yValueObserved - yValuePredicted)^2). PRESS helps us calculate RMSEP which is the root mean squared error of prediction. RMSEP finds the average difference between the observed value and predicted value for y. We want that value to be low for a smaller deviation from the observed value. RMSEP = sqrt(PRESS/ numberOfRowsInTestingData). The SST is the total sum of squares and is sum((yValuePredicted - mean(yValueObserved))^2), which is the difference between the observed data points from the observed mean value. It is used to calculate R^2 which is the coefficient of determination. R^2 = 1 - (PRESS/SST). R^2 helps us determine the variability of y. With values close to 1, that means there is a greater fit of the model with the data. Essentially R^2 helps determine how well the model has fitted the data and you want to get that number close to 1 to show the strength of the relationship between x and y variable and how strongly the independent variable can predict the dependent variable. RMSEP tells us the average difference between the observed value and predicted value for y and we want to get that lower for salary difference in numbers and great model accuracy."
  })
  output$text3.1 <- renderText({
    "Discovery: I did research with employers and recruiters to find articles such as the one from Salary.com, US Bureau of Labor Statistics, and sites (mentioned in references) to see if Age or YearsExpereince impact salary. Within this business domain, recruiters and employers see both Age and YearsOfExperience as predictors to whether they can trust the employee to successfully and thoroughly complete the tasks assigned. Thus these are the right factors to focus on (Age and YearsOfExperience). Hypothesis: Age or YearsOfExperience working have a strong relationship with Salary and can be used to predict Salary close to the actual Salary number. Pain Points: As an employee and job searcher, I want to know how to evaluate a salary against industry standard so I am not undervalued and paid less. Goal: Understand the trend and model between Age and Salary or YearsExperience and Salary to predict Salary based on Age or YearsExperience.Risks: The salary range may fluctuate based on location and industry so the predictions need to be scaled based on other factors like cost of living, value addition, and age of industry. Stakeholders: jobseekers to determine a fair salary and recruiter and employers to determine a fair salary to pay to underpaid workers and attract talent to join their company for fair pay and salary.  
"
  })
